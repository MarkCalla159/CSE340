...
invCont.processClassification = async function (req, res, next){
  ...
  if (classResult){
    ...
    req.flash(
      "notice", 
      "Sorry, the new classification failed")
...
invCont.processNewInventory = async function(req, res, next){
  ...
  const addResult = await invModel.addNewInventory(classification_id, 
    inv_make,
    inv_model,
    inv_year,
    inv_description,
  ...
  if (addResult) {
    req.flash(
      "notice",
      `Congratulations, you added the: ${inv_make + ' ' + inv_model} classification.`
    )
    let nav =await utilities.getNav();
    res.status(201).render("./inventory/management", {
      title: "Vehicle Management",
    ...
    req.flash(
      "notice",
      `Sorry, the new vehicle could not be added`
    )
